...
const ramshadsha = require("./ramshadsha");
const shamwol = require('./shamwol');
const { FakeDB, takeMessage } = require("./plugins/sql/fake");
const chalk = require('chalk');
const config = require('./config');
const simpleGit = require('simple-git');
const {WAConnection, MessageOptions, MessageType, Mimetype, Presence} = require('@adiwajshing/baileys');
const {Message, StringSession, Image, Video} = require('./SHAHMWOL/');
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('✅️ SHAHMWOL working ' + config.WORKTYPE + ' 𝚗𝚘𝚠 '));
          // thanks to afnanplk
...
*/
                        try {
                            ...
                            if (config.NOLOG == 'off') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*~_________~ ꌚꍩꁲꍩꂵꅏꂦ꒒ ~______~*' +
                                    '\n\n*😵 ' + error + '*\n\n```Report errors\njoin ⚠️Warning bot not allowed in the group\https://chat.whatsapp.com/F7p4E5x9972EcD9w6pAPYg``` ' 
                                    , MessageType.text);
                            }
                        }
                    }
